...
public class Pattern {
    ...
    public Pattern(String pattern) { this.pattern = pattern; }

    public boolean match(long value){

        for (int i = 0; i < pattern.length(); i++) {

            if (pattern.charAt(i) != 'x' && pattern.charAt(i) != 'z'
                && Integer.parseInt("" + pattern.charAt(i)) != ((value>>(pattern.length() - i - 1)) & 1)) {
                return false;
            }
