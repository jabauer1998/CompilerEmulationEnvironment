...
public class Parser {
	...
	private Token skip(){
		...
		errorLog.addItem(new ErrorItem("Unexpected end of file while skipping token", null));
		errorLog.printLog();
		System.exit(1);
		return null;
	}
	...
	private Token peek(){
		...
		errorLog.addItem(new ErrorItem("Unexpected end of file while peeking at token", null));
		errorLog.printLog();
		System.exit(1);
		return null;
	}
	...
	private Token match(Token.Type type){
		...
		if (lexedTokens.isEmpty()) { // if token isnt matched then
			...
			errorLog.addItem(
				new ErrorItem("Token of type " + type + " expected but token of type " + matched.getTokenType() + " found ",
					matched.getPosition()));
			errorLog.printLog();
			System.exit(1);
			return null;
	...
	private ModItem parseModItem(){
		...
		} else if (willMatch(Token.Type.ANDGATE) || willMatch(Token.Type.ORGATE) || willMatch(Token.Type.NANDGATE)
			|| willMatch(Token.Type.NORGATE) || willMatch(Token.Type.NOTGATE) || willMatch(Token.Type.XNORGATE)
			|| willMatch(Token.Type.XORGATE)) {
			return parseGateDeclaration();
		} else {
			Token matched = peek();
			errorLog.addItem(new ErrorItem(
				"Unexpected ModItem token of type " + matched.getTokenType() + " and lexeme " + matched.getLexeme() + " found",
				matched.getPosition()));
			errorLog.printLog();
			System.exit(1);
			return null;
	...
	private ModItemList parseModItemList(){
		...
		while(!willMatch(Token.Type.ENDMODULE)) { modList.add(parseModItem()); }
	...
	public Statement parseStatement(){
		...
		if (willMatch(Token.Type.IF)) {
			...
			if (willMatch(Token.Type.EQ1)) { // it is a blocking assignment
				...
				Token matched = peek();
				errorLog.addItem(new ErrorItem("Unexpected Statement token of type " + matched.getTokenType() + " and lexeme "
					+ matched.getLexeme() + " found", matched.getPosition()));
				errorLog.printLog();
				System.exit(1);
				return null;
	...
	/**
	 * Below is the code for parsing expressions for the verilog lanuage. These recursive
	 * decent takes into account all operator precedence and it can also be used to parse
	 * strings There are no booleans in Verilog so having a booelan would not make a bunch
	 * of sense.
	 * 
	 * @author Jacob Bauer
	 */
	...
	private Expression parseUNARY_Expression(){
		...
		if (willMatch(Token.Type.PLUS) || willMatch(Token.Type.MINUS) || willMatch(Token.Type.BNEG) || willMatch(Token.Type.LNEG)
			|| willMatch(Token.Type.BAND) || willMatch(Token.Type.BNAND) || willMatch(Token.Type.BOR)
			|| willMatch(Token.Type.BXOR) || willMatch(Token.Type.BXNOR)) {
			Token op = skip();
