...
public class Interpreter implements StatementVisitor<Void>, ModuleVisitor<Void> {

	private final ErrorLog errorLog;
	private Environment environment;

	private DeepVisitor deepVisitor = new DeepVisitor(errorLog, environment);
	private ShallowVisitor shallowVisitor = new ShallowVisitor(errorLog, environment);

	public Interpreter(ErrorLog errorLog, Environment environment){
		this.errorLog = errorLog;
		this.environment = environment;
		this.deepVisitor =  new DeepVisitor(errorLog, environment);
		this.shallowVisitor = new ShallowVisitor(errorLog, environment);
	}

	public Interpreter(ErrorLog errorLog) {
		this(errorLog, new Environment());
	}
	...
	public Void visit(ModuleDeclaration mod, Object... argv){
		...
		if (environment.moduleExists(modName)) {
			errorAndExit("Redeclaration of Module " + modName + "found at " + '[' + mod.getPosition() + "] declared ", env.lookupModule(modName));
		} else {
			environment.addModule(moduleName, mod);
		}
	...
	public Void visit(FunctionDeclaration function, Object... argv){
		...
		funcEnv.addEntry(getcallStackName(), new InterpreterFunctionData(function, function.getPosition()));
	...
	public Void visit(Assignment assign, Object... argv){
		...
		if (lValue instanceof VectorElement) {
			name = ((VectorElement)lValue).getIdentifier();
		...
		if (data.getObject() instanceof Double) {
			...
			int index = (int)((VectorElement)lValue).getExpression().accept(this);
			long value = 0;
			...
			} else if (lValue instanceof VectorElement) {
				int index = (int)longValue(((VectorElement)lValue).getExpression().accept(this));
	...
	public Void visit(BlockAssign assign, Object... argv){
		...
		if (result instanceof Scanner) {
			...
			Long[] arr = (Long[])data.getObject();
			int index = (int)(long)((VectorElement)lValue).getExpression().accept(this);
			long value = 0;
		...
		if (inFunctionReturn && name.getLexeme().equals(getcallStackName())) { changeTopExit(true); }
