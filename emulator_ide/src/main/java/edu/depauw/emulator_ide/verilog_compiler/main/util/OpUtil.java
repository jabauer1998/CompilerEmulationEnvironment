...
public class OpUtil {
    ...
    /** I created two types of assignments for verilog and the difference breaks down to how registers are assigned to one another
     * In a shallow assignment the value is copied over from one register to another a register can hold a True or a False value representing whether the output from that register is high or low
     * So the assignment works by assigning the value from one register to the value of the current register. This is different then actually changing the literal register object in a deep assignment.
     ...
     * This is useful for a continuous assignment in verilog. As opposed to observing the expression on the right hand side with a loop and coppying changes to the left hand side I am actually making the objects that are on the right hand side the things that are on the left hand side so changes come across in both variables.
     * @author Jacob Bauer
     */
    ...
    public static void shallowAssign(CircuitElem elem1, Vector<CircuitElem> vec2){
    	...
    	((Register)elem1).setSignal(vec2.getValue(index).getSignal());
    }
    ...
    public static void shallowAssign(CircuitElem elem1, long int2){
    	((Register)elem1).setSignal((int2 & 1) > 0);
    }
    ...
    public static void shallowAssign(CircuitElem elem1, boolean bool2){
    	((Register)elem1).setSignal(bool2);
    }

    public static void shallowAssign(CircuitElem elem1, CircuitElem elem2){
    	((Register)elem1).setSignal(elem2.getSignal());
    }

    public static long toLong(CircuitElem elem){
