...
public class TypeChecker implements ExpressionVisitor<TypeCheckerVariableData.Type>, StatementVisitor<Void>, ModuleVisitor<Void>,
	...
	public Void visit(ModuleDeclaration mod, Object... argv){
		...
		if (modEnv.entryExists(modName.getLexeme())) {
			errorLog.addItem(new ErrorItem(
				"Redeclaration of Module " + modName.getLexeme() + "found at " + '[' + mod.getPosition() + "] declared ",
	...
	public Void visit(FunctionDeclaration function, Object... argv){
		...
		if (funcEnv.entryExists(topFunctionName.getLexeme())) {
			errorLog.addItem(new ErrorItem("Duplicate function " + topFunctionName.getLexeme() + " allready exists ",
				topFunctionName.getPosition()));
		} else {
			funcEnv.addEntry(topFunctionName.getLexeme(),
				new TypeCheckerFunctionData(varEnv.getEntry(topFunctionName.getLexeme()), function.getPosition()));
		}
	...
	public Void visit(TaskDeclaration task, Object... argv){
		...
		if (funcEnv.entryExists(taskName.getLexeme())) {
			errorLog.addItem(new ErrorItem("Task declaration by the name of " + taskName.getLexeme() + " found at ["
				+ taskName.getPosition() + "] already exists at ", funcEnv.getEntry(taskName.getLexeme()).getPosition()));
		} else {
		...
		for (int i = 0; i < decl.numIdentifiers(); i++) {
			...
			if (varEnv.entryExists(current.getLexeme())) {
				...
				if (entryData.type == TypeCheckerVariableData.Type.UNDEFINED) {
					...
					errorLog.addItem(new ErrorItem("Cannot reassign variable of type " + entryData.type + " to type "
						+ TypeCheckerVariableData.Type.INPUT_WIRE, current.getPosition()));
				}
				...
				TypeCheckerVariableData data = new TypeCheckerVariableData(TypeCheckerVariableData.Type.INPUT_WIRE,
					current.getPosition());
	...
	public TypeCheckerVariableData.Type visit(BinaryOperation op, Object... argv){
		...
		if (left == TypeCheckerVariableData.Type.UNDEFINED) {
			...
			switch(op.getOp().getOpType()){
				...
				default:
					errorLog.addItem(
						new ErrorItem("Invalid operation in verilog of type " + left + " " + op.getOp().getOpType() + " " + right,
							op.getPosition()));
					return TypeCheckerVariableData.Type.UNDEFINED;
			}
	...
	public TypeCheckerVariableData.Type visit(UnaryOperation op, Object... argv){
		...
		if (op.getOp().getOpType() == UnOp.Type.PLUS
		...
		} else if (op.getOp().getOpType() == UnOp.Type.BNEG && (right == TypeCheckerVariableData.Type.STRING
