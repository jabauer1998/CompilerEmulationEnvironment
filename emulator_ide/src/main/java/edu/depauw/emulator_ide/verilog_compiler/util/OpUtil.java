package edu.depauw.emulator_ide.verilog_compiler.util;
...
public class OpUtil {
	...
	private boolean caseBoolean(LongVal target, Value Val){
	...
	public static void shallowAssign(Vector vec1, Vector vec2){
		if (vec1.getSize() == vec2.getSize()) {
			...
			while(sIndex1 <= end1) {
				Register elem1 = vec1.getValue(sIndex1);
				boolean signal = vec2.getValue(sIndex2).getStateSignal();
				elem1.setSignal(signal);
				sIndex1++;
				sIndex2++;
