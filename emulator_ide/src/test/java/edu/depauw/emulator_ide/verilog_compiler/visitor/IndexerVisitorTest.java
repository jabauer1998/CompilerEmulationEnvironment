...
public class IndexerVisitorTest {
	...
	public void testFullAdder(){
		 ...
		 * 
		 * Destination display = new Destination(new StringWriter()); Source source = new
		 * Source(new StringReader(input)); InfoLog errorLog = new InfoLog(display); Lexer lex =
		 * new Lexer(source, errorLog); Parser parse = new Parser(lex.tokenize(), errorLog);
		 * ModuleDeclaration moddec = parse.parseAST(); IndexerVisitor idVisitor = new
		 * IndexerVisitor(moddec, display, errorLog); idVisitor.visitRoot();
		 */
	}
}
